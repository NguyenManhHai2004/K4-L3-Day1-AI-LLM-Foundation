...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    headers = ["Prompt", "GPT-4o Response", "Mini Response", "GPT-4o (s)", "Mini (s)"]
    col_w = [40, 40, 40, 10, 10]
    ...
    for r in results:
        ...
        g = r.get("gpt4o_response", "")
        g_str = (g[:37] + "...") if len(g) > 40 else g
        m = r.get("mini_response", "")
        m_str = (m[:37] + "...") if len(m) > 40 else m
        gl = f"{r.get('gpt4o_latency', 0.0):.2f}"
        ml = f"{r.get('mini_latency', 0.0):.2f}"

        row = [p_str, g_str, m_str, gl, ml]
        lines.append(" | ".join(f"{val:<{w}}" for val, w in zip(row, col_w)))
